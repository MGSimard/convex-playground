...
function CommandEmpty({ ...props }: React.ComponentProps<typeof CommandBase.Empty>) {
  return <CommandBase.Empty className="py-6 text-center text-sm" {...props} />;
}
...
function CommandSeparator({ className, ...props }: React.ComponentProps<typeof CommandBase.Separator>) {
  return <CommandBase.Separator className={cn("bg-border -mx-1 h-px", className)} {...props} />;
}
...
function CommandShortcut({ className, ...props }: React.ComponentProps<"span">) {
  return (
    <span
      className={cn("text-muted-foreground ml-auto flex items-center gap-1 text-xs tracking-widest", className)}
      {...props}
    />
  );
}
